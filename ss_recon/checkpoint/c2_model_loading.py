...
def convert_basic_c2_names(original_keys):
    ...
    # layer1-4 is used by torchvision, however we follow the C2 naming strategy
    # (res2-5)
...
def convert_c2_detectron_names(weights):
    ...
    # --------------------------------------------------------------------------
    # FPN case
    # In the C2 model, the RPN hidden layer conv is defined for FPN level 2 and
    # then
    ...
    def fpn_map(name):
        ...
        if name.startswith("fpn.inner."):
            # splits example: ['fpn', 'inner', 'res2', '2', 'sum', 'lateral',
            # 'weight']
    ...
    for orig, renamed in zip(original_keys, layer_keys):
        ...
        if renamed.startswith("bbox_pred.") or renamed.startswith(
            ...
            logger.info(
                "Remove prediction weight for background class in {}. "
            ...
            logger.info(
                "Move classification weights for background class in {} "
                "from index 0 to "
...
def align_and_update_state_dicts(
    ...
    that the original pre-trained weights from ImageNet won't contain.
    For example, model.state_dict() might return
    backbone[0].body.res2.conv1.weight, while the pre-trained model contains
    res2.conv1.weight. We thus want to match both parameters together.
    For that, we look for each model weight, look among all loaded keys if there
    is one
    that is a suffix of the current weight name, and use it if that's the case.
    If multiple matches exist, take the one with longest size
    of the corresponding name. For example, for the same model as before, the
    pretrained
    weight file can contain both res2.conv1.weight, as well as conv1.weight. In
    this case,
