...
class RandomMRIMultiShotMotion(TransformGen):
    ...
    def __init__(
        self,
        seq_tfm_gens: Union[Sequence[TransformGen], Sequence[Sequence[TransformGen]]],
        p: float = 0.0,
    ):
        super().__init__(params={"seq_tfm_gens": seq_tfm_gens}, p=p)

    def get_transform(self, input: torch.Tensor):
        params = self._get_param_values(use_schedulers=True)
        if self._rand() >= params["p"]:
            return NoOpTransform()
        tfm_gens = self._rand_range(*params["seq_std_devs"])
        gen = self._generator
        if gen is None or gen.device != input.device:
            gen = torch.Generator(device=input.device).manual_seed(int(self._rand() * 1e10))
        return MRIMultiShotMotion(tfm_gens=tfm_gens, generator=gen)
