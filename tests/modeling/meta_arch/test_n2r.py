...
from meddlr.modeling.meta_arch import N2RModel
...
class N2RModelTest(unittest.TestCase):
    ...
    def _build_inputs(
        self, ky: int, kz: int, supervised_bsz: int, unsupervised_bsz: int
    ...
    def test_forward(self, supervised_bsz: int, unsupervised_bsz: int):
        ...
        inputs = self._build_inputs(ky, kz, supervised_bsz, unsupervised_bsz)
        ...
        if unsupervised_bsz > 0:
            ...
            np.testing.assert_allclose(cons_target, expected_targets, rtol=1e-5, atol=1e-5)
