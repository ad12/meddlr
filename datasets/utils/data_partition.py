...
def _random_balance(bins: List[List[Any]], expected_dist: Union[List[float], np.ndarray], buffer: float,
                    weights_dict: dict,
                    iters: int = 100):
    ...
    assert len(bins) == len(expected_dist), \
    ...
    for _ in range(iters):
        for c_ind in range(len(bins)):
            ...
            # Select candidate bin. If c_bin is overfilled (delta > 0), pick an underfilled bin. And vice versa.
            candidate_indexes = np.argwhere(curr_weights - expected_dist < 0).flatten() if is_bin_overfilled else \
            ...
            candidate_elems_idxs = np.argwhere(candidate_bin_weights < ce_weight).flatten() if is_bin_overfilled else \
...
def _greedy_balance(bins: List[List[Any]], expected_dist: Union[List[float], np.ndarray], buffer: float,
                    weights_dict: dict):
    ...
    sorted_bins = [sorted([(val, weights_dict[val]) for val in c_bin], key=lambda x: x[-1]) for c_bin in bins]
    ...
    while True:
        ...
        candidate_elems = []
        candidate_weight = 0
        for ind, elem in enumerate(of_bin):
...
def approximately_split_weighted(
    ...
    This function should be used to split :obj:`values` into N disjoint sets. This is useful for applications in machine
    learning where dataset splits should be disjoint.

    In certain cases, values will not have uniform weights. For example, consider the case where there are frames
    extracted from 3 videos: video A- 10 frames, video B- 5 frame,s video C- 2 frames. In this case, we would like to
    keep frames sampled from the same video within a specific set. For example, frames from video A cannot be in both
    bin 1 and bin 2. The :obj:`weights` attribute can be specified to keep the videos separate::

        >>> approximately_split(values=["videoA", "videoB", "videoC"], weights=[10, 5, 2], ...)

    In these cases, because of the randomness of splits, we may end up with a lop-sided split. Let's consider the
    example above, where the ``split_percentages=[0.3, 0.7]``. Assume after random shuffling values are in the order
    ``["videoA", "videoB", "videoC"]``. In this case, no value will be assigned to the first bin. Balancing will allow
    bins to randomly or greedily reconfigure into an appropriate. This method is not guaranteed to converge.

    In random balancing, entries between overfilled and underfilled bins are pseudo-randomly swapped. Bins are sequentially
    selected. If the current bin is overfilled, a candidate bin that is underfilled is selected at random. If the current bin
    is underfilled, a candidate bin that is overfilled is selected. Then an element from the overfilled bin with a weight greater 
    than that of the other element is randomly selected and the two elements are swapped. This is repeated at most :obj:`iter`
    times or until bin is within buffer.

    In greedy balancing, the bin that is most underfilled is filled greedily iteratively from overfilled bins
    just enough so that it meets the buffer criteria. To fill the underfilled bin to as close to its size, set
    :obj:`buffer=0`.
    ...
    Args:
        ...
        split_percentages (:obj:`List[float]`): The approximate percent split between datasets. Must add up to 1.
        weights (:obj:`List[int]`, optional): Weights for each value in :obj:`values`. Use if certain values have a
            larger weight than others (i.e. more examples associated with value).
            Defaults to weight of 1 per each value in :obj:`values`.
        balance (:obj:`str`, optional): Balance weighting after partition. Options: `"greedy"`, `"random"`.
            Use with :obj:`weights`. Defaults to ``None``.
        buffer (:obj:`float`, optional): If bin percentages are within +/-:obj:`buffer`, the split is valid.
            Defaults to 0.01 (+/-1%).
        num_iters (:obj:`int`, optional): Number of iterations for random balancing. Defaults to ``100``.
    ...
    if weights:
        ...
        if len(weights) != num_values:
            raise ValueError("{} weights found, but {} values provided".format(len(weights), num_values))
    ...
    assert all([len(set(outputs[i]) & set(outputs[j])) == 0
